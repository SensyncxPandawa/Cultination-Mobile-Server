...
def generate_and_send_otp_code(db: Session, user_id: int):
    ...
    existing_user = db.query(models.UsersAuth).filter(models.UsersAuth.user_id == user_id).first()
    ...
    existing_user_2fa = db.query(models.Users2FA).filter(models.Users2FA.user_id == user_id,).first()
...
def validate_otp_code(db: Session, provided_otp_code: OTPCode, user_id: int):
    ...
    if user_2fa.otp_code == provided_otp_code:
        return "Success!"
