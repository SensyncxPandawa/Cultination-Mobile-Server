...
@router.post("/users/address/pond_address/{user_id}", response_model=List[PondsSchema], tags=["Users' Address"])
def create_pond_address(user_ponds: PondsSchema, db: Session = Depends(get_db)):
    return

# DISPLAY USER PRIMARY ADDRESS
@router.get("/users/address/primary_address/{user_id}", response_model=List[PrimarySchema], tags=["Users' Address"])
def display_existing_user_primary_address(user_id: int, db: Session = Depends(get_db)):
    return

# DISPLAY ALL USER POND ADDRESSES
@router.get("/users/address/pond_address/{user_id}", response_model=List[PondsSchema], tags=["Users' Address"])
def display_all_existing_user_pond_address(user_id: int, db: Session = Depends(get_db)):
    return

# DISPLAY CERTAIN USER POND ADDRESS
@router.get("/users/address/pond_address/{user_id}/{pond_address_id}", response_model=List[PondsSchema], tags=["Users' Address"])
def display_certain_existing_user_pond_address(user_id: int, db: Session = Depends(get_db)):
    return

# SET PRIMARY ADDRESS
@router.put("/users/address/primary_address/{user_id}", response_model=PrimarySchema, tags=["Users' Address"])
...
@router.put("/users/address/pond_address/{user_id}/{pond_address_id}", response_model=PondsSchema, tags=["Users' Address"])
...
@router.delete("/users/address/pond_address/{user_id}/{pond_address_id}", response_model=PondsSchema, tags=["Users' Address"])
