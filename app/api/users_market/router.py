...
# DIPLAY USER DATA
@router.get("/users/market/production_capacity/{user_id}", response_model=List[MarketSchema], tags=["Users"])
def display_existing_user_production_capacity(user_id: int, db: Session = Depends(get_db)):
    return

@router.get("/users/market/market_capacity/{user_id}", response_model=List[MarketSchema], tags=["Users"])
def display_existing_user_market_capacity(user_id: int, db: Session = Depends(get_db)):
    return

@router.get("/users/market/market_preference/{user_id}", response_model=List[MarketSchema], tags=["Users"])
def display_existing_user_market_preference(user_id: int, db: Session = Depends(get_db)):
    return

# SET USER DATA
@router.put("/users/market/{user_id}", response_model=MarketSchema, tags=["Users"])
def update_user_market_by_id(user_id: int, updated_user_market: MarketSchema, db: Session = Depends(get_db)):
    return

@router.put("/users/market/production_capacity/{user_id}", response_model=MarketSchema, tags=["Users"])
def update_user_production_capacity_by_id(user_id: int, updated_user_market: MarketSchema, db: Session = Depends(get_db)):
    return

@router.put("/users/market/market_capacity/{user_id}", response_model=MarketSchema, tags=["Users"])
def update_user_market_capacity_by_id(user_id: int, updated_user_market: MarketSchema, db: Session = Depends(get_db)):
    return

@router.put("/users/market/market_preference/{user_id}", response_model=MarketSchema, tags=["Users"])
