...
from .schemas import UsersMarket, UserProductionCapacity, UserMarketCapacity, UserMarketPreference
...
def update_user_market_by_id(user_id: int, updated_user_market: UsersMarket, db: Session = Depends(get_db)):
...
def update_user_production_capacity_by_id(user_id: int, updated_user_market: UserProductionCapacity, db: Session = Depends(get_db)):
    return services.update_user_market_by_id(db, user_id, updated_user_market)

@router.put("/users/{user_id}/market/market_capacity", response_model=UserMarketCapacity, tags=["Users' Market"])
def update_user_market_capacity_by_id(user_id: int, updated_user_market: UserMarketCapacity, db: Session = Depends(get_db)):
    return services.update_user_market_by_id(db, user_id, updated_user_market)

@router.put("/users/{user_id}/market/market_preference", response_model=UserMarketPreference, tags=["Users' Market"])
def update_user_market_preference_by_id(user_id: int, updated_user_market: UserMarketPreference, db: Session = Depends(get_db)):
    return services.update_user_market_by_id(db, user_id, updated_user_market)
