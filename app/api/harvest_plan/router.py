...
@router.post("/users/harvest_plan/{user_id}", response_model=List[HarvestSchema], tags=["Users"])
def create_user_harvest_plan(user_auth: HarvestSchema, db: Session = Depends(get_db)):
    return

# DISPLAY USER HARVEST PLAN
@router.get("/users/harvest_plan/{user_id}", response_model=List[HarvestSchema], tags=["Users"])
def display_all_existing_user_harvest_plan(user_id: int, db: Session = Depends(get_db)):
    return

# UPDATE USER HARVEST PLAN
@router.put("/users/market/{user_id}/{harvest_plan_id}", response_model=HarvestSchema, tags=["Users"])
...
@router.delete("/users/harvest_plan/{user_id}/{harvest_plan_id}", response_model=HarvestSchema, tags=["Users"])
