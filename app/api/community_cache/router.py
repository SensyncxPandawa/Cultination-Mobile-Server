...
@router.put("/users/community/harvest_cache/update/{harvest_plan_id}", response_model=CommunitySchema, tags=["Community Cache"])
...
@router.get("/users/community/harvest_cache", response_model=List[CommunitySchema], tags=["Community Cache"])
def display_overview_community_cache_this_month(user_id: int, db: Session = Depends(get_db)):
    return

@router.get("/users/community/harvest_cache/province/{community_province}", response_model=List[CommunitySchema], tags=["Community Cache"])
def display_overview_community_cache_this_month_by_province(user_id: int, db: Session = Depends(get_db)):
    return

@router.get("/users/community/harvest_cache/city/{community_city}", response_model=List[CommunitySchema], tags=["Community Cache"])
def display_overview_community_cache_this_month_by_city(user_id: int, db: Session = Depends(get_db)):
    return

@router.get("/users/community/harvest_cache/fish_type/{community_fish_type}", response_model=List[CommunitySchema], tags=["Community Cache"])
def display_overview_community_cache_this_month_by_fish_type(user_id: int, db: Session = Depends(get_db)):
    return

@router.get("/users/community/harvest_cache/month/{community_month}", response_model=List[CommunitySchema], tags=["Community Cache"])
